...
var resolve = exports.resolve = function resolve ( socket, server, callback, retransmission ) {
  ...
  test.udp(socket, server, count, timeout, function ( error, value ) {
    ...
    if ( !other || retransmission.short ) return callback(null, label(result));

    if ( !test.compare(result.public, result.local) ) {
      var otser = { host: other.value.obj.host, port: server.port }
...
var resolve_tcp = exports.resolve_tcp = function resolve_tcp ( socket, server, callback, retransmission ) {
  return resolve(tcp2udp(socket), server, callback, retransmission);
}
